...
def order_summary():
	...
	for i in range(len(order_lst)):
		if order_lst[i][0] != None:
			...
			print('{:24s} | {:3d} | {:7d}'.format(order_lst[i][0][0], order_lst[i][1],
												  (int(order_lst[i][0][1])) * order_lst[i][1]))
...
def order():
	...
	if option == 2:
		...
		for i in range(3):
			...
			check_addon = check_avail(addon_item)
			if check_addon == 0:
				# break
				return order()
		order_summary()
...
def main():
	start = int(input('press 1 to start order:'))
	while True:
		if start == 1:
			o = order()
			if o != 0:
